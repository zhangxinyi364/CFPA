...
######################################################
SEED=28465820
CATEGORY=Chair  # choices=['Chair', 'Table', 'Lamp']
DATA_TRAIN=Chair.train.npy
DATA_VAL=Chair.val.npy
LEVEL=3
# Training.
LR=1.5e-4
WEIGHT_DECAY=1e-4
...
# Others.
BASE_DIR=${CURDIR}
OUTPUT_DIR=${BASE_DIR}/checkpoints/inference     
VIS_DIR=${BASE_DIR}/visualization_results   
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --master_port=$((RANDOM + 10000)) --use_env --nproc_per_node=${NUM_GUPS} ${CURDIR}/execute.py --category ${CATEGORY} \
...
--eval-epochs 0 --eval-only 1 --checkpoint ${CHECKPOINT} \
-eb 16 \
--save-dir ${VIS_DIR} \
--save-vis ${SAVE_VIS}