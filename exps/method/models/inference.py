...
from .func import (linear_assignment,
                   ...
                   get_contact_point_loss_for_single_part,get_group_symmetric_loss)
...
def infer_execute(pred_poses_per_trans, gt_poses, part_pcs, part_valids, match_ids,
                  contact_points, sym_info, args):

    batch_size, num_part, _ = pred_poses_per_trans.size()
    ...
    for bs_ind in range(batch_size):
        ...
        for ins_id in range(1, num_part + 1):
            ...
            matched_pred_ids, matched_gt_ids = linear_assignment(
                cur_pts, cur_pred_centers, cur_pred_quats,
    ...
    trans_l2_loss = get_trans_l2_loss(pred_trans, gt_trans, part_valids,mse_weight)
    rot_cd_loss = get_rot_cd_loss(part_pcs, pred_rot, gt_rot, part_valids)
    shape_cd_loss = get_shape_cd_loss_default(part_pcs, pred_rot, gt_rot, pred_trans, gt_trans, part_valids)
    total_cd_loss, acc = get_total_cd_loss(part_pcs, pred_rot, gt_rot, pred_trans, gt_trans, part_valids)


    contact_point_loss, num_correct_contact, num_contact_point = get_contact_point_loss(
    ...
    if sym_info is not None:
        ...
        total_cd_loss = total_cd_loss + group_sym_loss * args.super_part_weight

    return acc, trans_l2_loss, rot_cd_loss, pred_poses_per_trans, \
        total_cd_loss, shape_cd_loss, contact_point_loss, \
        num_correct_contact, num_contact_point
...
def inference(pred_poses, gt_poses, part_pcs, part_valids, match_ids, contact_points, sym_info, args=None):
    ...
    acc, trans_l2_loss_per_trans, rot_cd_loss_per_trans, pred_poses_per_trans, \
        total_cd_loss_per_trans, shape_cd_loss_per_trans, contact_point_loss_per_trans, \
        num_correct_contact, num_contact_points = infer_execute(
        pred_poses_per_trans, gt_poses, part_pcs, part_valids, match_ids,
        contact_points, sym_info, args)


    acc2, *rest2 = infer_execute(
        pred_poses_per_trans2, gt_poses, part_pcs, part_valids, match_ids,
        contact_points, sym_info, args)

    return (acc, trans_l2_loss_per_trans, rot_cd_loss_per_trans, pred_poses_per_trans,
            total_cd_loss_per_trans, shape_cd_loss_per_trans, contact_point_loss_per_trans,
            num_correct_contact, num_contact_points)
