...
from .models import MLP, Predictor, FinalPoseHead, TransformerEncoder, TransformerEncoderLayer, TransformerDecoder, \
    ...
    def forward(self, cross, in_src, mask, gt_part_poses, base_pred, pos_embed, **kwargs):
        ...
        if self.args.ins_version == "v1":
            ...
            if self.args.ins_cat_inter_only:
                ins_codes = part_ids.new_zeros((batch_size, num_part, num_part * 2)).scatter_(2,
                                                                                              cate_codes.unsqueeze(2),
                                                                                              1)
                ins_codes[..., :num_part][valid_mask] = 0.
        ...
        if self.mode == 'base':
            preds, memory = self.encoder(match_codes, cross, base_src.permute(1, 0, 2), init_pred, class_codes,
                                         src_key_padding_mask=base_mask, pos=pos_embed, lens=lens,
                                         gt_part_poses=gt_part_poses, **kwargs)
            ...
            preds, memory = self.encoder(match_codes, cross, src, base_pred, ins_codes, mask=None,
                                         src_key_padding_mask=mask, pos=pos_embed, lens=lens,
                                         gt_part_poses=gt_part_poses, **kwargs)
            preds = preds.permute(2, 0, 1, 3)
...
class EnDe(nn.Module):
    ...
    def forward(self, in_src, mask, gt_part_poses, base_pred, pos_embed, **kwargs):
        ...
        preds, memory = self.encoder(src, base_pred, ins_codes, mask=None, src_key_padding_mask=mask, pos=pos_embed,
                                     lens=lens, gt_part_poses=gt_part_poses, **kwargs)
