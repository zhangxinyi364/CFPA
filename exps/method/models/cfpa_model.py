...
from Point_otk.FeatureProcessor import FeatureProcessor
...
from models.interaction.cluster_node_interaction import ClusterNodeInteraction
...
class hierarchical_trans(nn.Module):
    def __init__(self, args):
        ...
        self.feat_extract = pointnet_cls(k=args.feat_dim)
        ...
        self.feature_processor = FeatureProcessor(
            num_super_parts=args.num_super_parts,  
            feat_dim=args.feat_dim
        )

  
        self.cluster_node_interaction = ClusterNodeInteraction(args)
    ...
    def branch1(self, part_pcs, part_valid, gt_part_poses, match_ids, part_ids, contact_points, sym_info):
        ...
        enhanced_feat, super_feat, P, ordered_feat, ordered_part_valid = self.feature_processor(
            base_feat,
        ...
        cluster_node_feat = self.cluster_node_interaction(
            node_feats=ordered_feat,         # [B, P, D]
            cluster_feats=super_feat,     # [B, S, D]
            contact_points=contact_points, # [B, P, P, 4]
            part_valids=ordered_part_valid if ordered_part_valid is not None else part_valid        # [B, P]
        )

        final_feat = self.fusion_layer(
            torch.cat([enhanced_feat, cluster_node_feat], dim=-1)
        )

  
        P_transpose = P.transpose(1, 2)
        final_feat = torch.bmm(P_transpose, final_feat)
        ...
        for mon_idx in range(self.args.train_mon):
            if self.args.bi_pn == 1:
                ...
                preds, memory = self.encoder(base_contact_points, final_feat, part_valid, gt_part_poses, None, None, **kwargs)

        return preds, memory, super_feat, sym_info, final_feat
    ...
    def forward(self, raw_part_pcs, part_valid, gt_part_poses, match_ids, part_ids,
        ...
        preds1, memory1, super_feat, sym_info, final_feat = self.branch1(raw_part_pcs, part_valid, gt_part_poses, match_ids,
                                                             part_ids, contact_points, sym_info)
        ...
        if self.training:
            ...
            loss_per_mon1, trans_l2_loss1, rot_l2_loss1, trans_cd_loss1, rot_cd_loss1, shape_cd_loss1, part_cd_loss1 = \
                comp_losses(preds1, gt_part_poses, raw_part_pcs, part_valid, match_ids, self.mse_weight,
                    super_feat=super_feat,  
                    sym_info=sym_info, 
                    args=self.args)
                ...
                if flag_valid:
                    ...
                    if prob < self.args.filter_thresh:
                        memory1, base_feat, part_pcs, raw_part_pcs, part_valid, gt_part_poses, base_preds1, part_ids, match_ids, contact_points, _, part_mask = \
                            self.prepare_filters_v1(memory1, base_feat, part_pcs, raw_part_pcs, part_valid,
                                                    gt_part_poses, base_preds1, part_ids, match_ids,
                                                    contact_points=contact_points)
            ...
            for mon_idx in range(self.args.train_mon):
                ...
                if self.args.bi_pn == 1:
                    ...
                    preds, memory2 = self.encoder2(memory1, base_feat, part_valid, gt_part_poses,
                                                   base_preds1.permute(1, 0, 2), None, **kwargs)
                    final_preds = preds
                loss_per_mon, trans_l2_loss, rot_l2_loss, trans_cd_loss, rot_cd_loss, shape_cd_loss, part_cd_loss = \
                    comp_losses(final_preds, gt_part_poses, raw_part_pcs, part_valid, match_ids, self.mse_weight,
                                super_feat=super_feat, sym_info=sym_info, args=self.args)
            ...
            if self.args.type_eval == "encoder":
                if self.args.bi_pn == 1:
                    trans_l2_loss_per_trans, rot_cd_loss_per_trans, part_cd_loss, shape_cd_loss, contact_point_loss, acc, valid, \
                        ...
                        self.inference_encoder(base_feat, base_feat2, raw_part_pcs, part_valid, gt_part_poses,
                                               base_preds1.permute(1, 0, 2),
                                               part_ids, match_ids, memory1, sym_info)
                else:
                    trans_l2_loss_per_trans, rot_cd_loss_per_trans, part_cd_loss, shape_cd_loss, contact_point_loss, acc, valid, \
                        num_contact_correct, num_contact_point, batch_size, pred_poses, cdsV1_sum, cdsV2_sum = \
                        self.inference_encoder(memory1, base_feat, base_feat, raw_part_pcs, part_valid, gt_part_poses,
                                               base_preds1.permute(1, 0, 2), base,
                                               part_ids, match_ids, contact_points, sym_info)
                output["pred_poses"] = pred_poses

                final_preds = pred_poses
                pred_rot = final_preds[:, -1, :, 3:]  
                pred_trans = final_preds[:, -1, :, :3]  
                gt_rot = gt_part_poses[:, :, 3:]  
                gt_trans = gt_part_poses[:, :, :3]  
            ...
            return trans_l2_loss_per_trans, rot_cd_loss_per_trans, part_cd_loss, shape_cd_loss, contact_point_loss, \
                acc, valid, num_contact_correct, num_contact_point, batch_size, {
                'pred_poses': final_preds,
                'pred_rot': pred_rot,
    ...
    def prepare_decoder_v1(self, part_feat, part_pcs, part_valid):
        ...
        pos_bs = torch.tensor([_ for _ in range(batch_size)]).unsqueeze(1).repeat(1, num_pos).view(-1).to(
            part_valid.device)
    ...
    def prepare_filters_v1(self, memory, part_feat, part_pcs, raw_part_pcs, part_valid, part_pose, base_preds1,
                           part_ids, match_ids,
        ...
        filter_bs = torch.tensor([_ for _ in range(batch_size)]).unsqueeze(1).repeat(1, num_filter).view(-1).to(
            part_valid.device)
        ...
        memory_filter = memory.clone().permute(1, 0, 2)[part_mask_3.repeat(1, 1, len_feat)].view(num_res, batch_size,
                                                                                                 len_feat).contiguous()
        new_part_feat = part_feat.clone()[part_mask_3.repeat(1, 1, len_feat)].view(batch_size, num_res,
                                                                                   len_feat).contiguous()
        new_part_pcs = part_pcs.clone()[part_mask_4.repeat(1, 1, num_point, 3)].view(batch_size, num_res, num_point,
                                                                                     3).contiguous()
        new_raw_part_pcs = raw_part_pcs.clone()[part_mask_4.repeat(1, 1, num_point, 3)].view(batch_size, num_res,
                                                                                             num_point, 3).contiguous()
        ...
        if contact_points is not None:
            contact_points2 = contact_points.clone()[part_mask_4.repeat(1, 1, num_part, 4)].view(batch_size, num_res,
                                                                                                 num_part,
                                                                                                 4).contiguous()
            part_mask_4_ = part_mask.unsqueeze(1).unsqueeze(-1).repeat(1, num_res, 1, 4)
            new_contact_points = contact_points2.clone()[part_mask_4_].view(batch_size, num_res, num_res,
                                                                            4).contiguous()
    ...
    def inference_encoder(self, cross, base_feat, base_feat2, part_pcs, part_valid, part_poses, base_pred, base,
                          part_ids, match_ids, contact_points, sym_info):
        ...
        for mon_id in range(self.args.eval_mon):
            ...
            acc_per_mon, trans_l2_loss_per_trans, rot_cd_loss_per_trans, pred_poses_per_mon, part_cd_loss_per_mon, shape_cd_loss_per_mon, contact_point_loss_per_mon, \
                contact_correct_per_mon, num_contact_point = inference(final_preds, part_poses, part_pcs, part_valid,
                                                                       match_ids, contact_points, sym_info,
                                                                       args=self.args)

            contact_point_loss_per_data, count, total_num, batch_count, batch_total_num = batch_get_contact_point_loss(
                final_preds[:, -1, :, :3], final_preds[:, -1, :, 3:], contact_points, sym_info)
