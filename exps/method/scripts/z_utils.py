...
def save_model(epoch, scaler, output_dir, model, optimizer, part_acc, best_acc, args, contact_acc=None):
    ...
    # 新增逻辑：检查contact accuracy值是否超过阈值
    ca_threshold = getattr(args, 'ca_threshold', 45.2)  # 默认阈值为48.5
    if contact_acc is not None and contact_acc > ca_threshold:
        if args.rank == 0:  # only the first GPU saves checkpoint
            save_checkpoint({
                'epoch': epoch + 1,
                'state_dict': model.state_dict(),
            ...
            }, is_best=False, filename=osp.join(output_dir, f'checkpoint_ca{contact_acc:.2f}_ep{epoch+1}.pth.tar'))
            print(f"Saved model with Contact Accuracy={contact_acc:.2f} at epoch {epoch+1}")
