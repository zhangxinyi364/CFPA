...
from Point_otk.models.containers import Module
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.reshape(b_s,1,1,nk).repeat(1,self.h,nk,1)
            att = att.masked_fill(attention_mask, -np.inf)
        att = torch.softmax(att, -1)
        if attention_mask is not None:
            att = att.masked_fill(attention_mask.transpose(-1,-2),0)
...
class MultiHeadAttention_BN(Module):
    '''
    Multi-head attention layer with Dropout and Layer Normalization.
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
                 attention_module=None, attention_module_kwargs=None):
        super(MultiHeadAttention_BN, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        if attention_module is not None:
            if attention_module_kwargs is not None:
                self.attention = attention_module(d_model=d_model, d_k=d_k, d_v=d_v, h=h, **attention_module_kwargs)
...
class MultiHeadAttention_BN2(Module):
    '''
    Multi-head attention layer with Dropout and Layer Normalization.
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
                 attention_module=None, attention_module_kwargs=None):
        super(MultiHeadAttention_BN2, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        if attention_module is not None:
            if attention_module_kwargs is not None:
                self.attention = attention_module(d_model=d_model, d_k=d_k, d_v=d_v, h=h, **attention_module_kwargs)
